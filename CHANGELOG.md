...
### 新增
- **`/jm` 改为由精确到宽松的分段搜索**。JM 标题会先清理作者、翻译和版本尾缀并搜索完整作品名；零结果或结果未通过创作者校验时，再按 `～`、`｜` 等标题边界尝试较长副标题，并从 `○` 等强分隔符前提取稳定锚点。实际命中的关键词会写入搜索状态，后续翻页继续沿用，不会重新跑候选链。
- **JM 作者与社团线索参与 EH 结果软校验**。读取 `album.authors`，并从 `[社团 (作者)]` 标题前缀补充线索；新增 `pykakasi`，按 EHWiki 默认的 revised Wāpuro Hepburn 风格生成日文作者罗马字，与 EH 的 `artist:` / `group:` 标签比较。匹配结果前置并显示“作者/社团匹配”；汉字多读音、标签缺失或转写不一致时仍保留第一组标题结果，不作硬过滤。

### 修复
- **残缺标题和过宽回退词不再制造误搜索**。清理 JM 截断元数据留下的孤立 `[`；拆分产生的候选至少保留 4 个有效字符并拒绝纯数字，避免 `1~10` 被拆成 `10` 后命中无关画廊。
- **EH 搜索统一遵守官方 3 秒间隔**。EH/ExHentai family 在统一搜索入口共享 3.05 秒节流，覆盖 `/ehsearch`、`/jm`、翻页、Ex→EH 回退及并发用户，降低连续候选触发 pageload 限制的风险。

### 验证
- 从 JM“單行本”标签的随机页面抽取 5 个排除图集/合集的真实样本：3 个找到人工核对正确的 EH 作品，2 个安全未命中；修复纯数字候选后未再出现明显误命中。新增 8 组专项回归，全量测试 58 项通过。

### 改动文件
- `pixivfeed/channel/telegram/handlers.py`
- `pixivfeed/provider/jm/__init__.py`
- `pixivfeed/provider/ehentai/_search.py`
- `tests/test_jm_search.py`
- `AGENTS.md`
- `pyproject.toml`：新增 `pykakasi>=2.3,<3`，版本 0.12.3 → 0.13.0
...
### 修复
- **归档发送失败不再误记为成功**。文件超过 Telegram 上限、限频重试耗尽、上传超时或 `sendDocument` 失败时，发送 helper 会把失败显式返回上层；`usage_log` 不再继续写 `ok` 和成功字节数。
