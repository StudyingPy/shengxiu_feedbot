...
- JM 与 EH 的同一作品标题常有繁简体、日文异体字、假名录入或截断尾缀差异。`/jm` 应先搜清洗后的完整标题；只有零结果或结果未通过作者/社团校验时，才按 `～` 等标题边界尝试较长副标题，再取 `○` 等强分隔符前的稳定锚点。校验线索来自 `album.authors` 和标题开头的 `[社团 (作者)]`，对比 EH 的 `artist:` / `group:`；日文作者需按 EHWiki 规则用 revised Wāpuro Hepburn 风格转写（`さいとう → saitou`），但汉字读音可能推断错误，因此原文/转写都只能用于前置和软校验，不能硬过滤。候选要去重、限制总数并在命中后立即停止，避免无谓增加 EH pageload；拆分产生的回退词至少保留 4 个有效字符且禁止纯数字（`1~10` 不能生成 `10`），翻页必须沿用实际命中的关键词。
- EHWiki 的 Gallery Searching 限制搜索至少间隔 3 秒，未说明 EH/ExHentai 额度独立。节流必须在 EH family 的统一搜索入口全局共享，覆盖 `/ehsearch`、`/jm`、翻页、Ex→EH 回退和并发用户，不能只在单个命令循环里局部 `sleep`。
