# Feed Bot

Telegram bot：把多个站点的链接自动转成 Telegra.ph 页面或直发图片。

支持的站点：

| 站点 | 内容类型 | 是否需要登录 |
| ---- | -------- | ------------ |
| Pixiv | illust（图片）/ novel（小说）| 仅 R-18 / 部分小说需要 PHPSESSID |
| e-hentai | gallery | 不需要 |
| ExHentai | gallery | 必须（ipb_pass_hash / ipb_member_id / igneous）|
| nhentai | gallery | 不需要 |

## 功能

### Pixiv

- 监听消息中的 Pixiv 链接，自动响应
- Illust 默认策略：图片数 ≤ `direct_threshold`（默认 5）直接发图，> 5 走 Telegra.ph
  - 直发 ≤ 10 张走 sendMediaGroup，> 10 张自动降级 Telegra.ph
  - `/pixiv_telegraph <链接>` / `/pixiv_direct <链接>` 强制模式
- Novel 自动转 Telegra.ph，支持 `[newpage]` / `[chapter:]` / `[[jumpuri:>]]` / `[pixivimage:]` / `[uploadedimage:]`
- Inline mode（仅 admin）：`@bot 12345` / `@bot artworks/12345` / `@bot novel/999` / 完整 URL

### e-hentai / ExHentai

私聊收到链接 → 显示标题、页数 → 弹出 4 模式按钮：

```
网页 · 显示图    |  网页 · 原图
归档 · 1280x   |  归档 · 原图
取消
```

- **网页 · 显示图**：sample 图，免 GP/Credits（DojinGo 默认行为）
- **网页 · 原图**：子页 "Download original" 链接，会消耗 GP/Credits
- **归档 · 1280x**：调 `archiver.php` 拿 zip，1280x 重采样，消耗免费 archive 配额
- **归档 · 原图**：同上，原始分辨率

群聊不弹按钮，使用 `collectors.{ehentai|exhentai}.default_mode` 配置的默认模式。

archive 模式失败（H@H 节点不可用 / 配额耗尽 / 超时）会报错，让用户改选其他模式。

### nhentai

API 走第三方镜像 `nhapi.cat42.uk`（DojinGo 同款）。直接 Telegra.ph 发布，多 CDN 自动 fallback。

### 白名单 + 运行时配置

- `/allow` `/deny` `/listallow` 热更新白名单
- `/setting list / get / set / edit / unset` 私聊改运行时配置（仅 admin）
  - `/setting list` 查看所有可改 key
  - `/setting set collectors.exhentai.enabled true`
  - `/setting set publish.direct_threshold 8`
  - `/setting edit templates.gallery.page_header`（多行模板）
  - 不可运行时改的字段：`telegram.token` / `storage.*` / `publish.base_url` / `auth.admin_users` / `publish.telegraph_token`

所有可改设置存 SQLite，重启不丢。优先级：runtime_settings > 环境变量 > config.yaml > 内置默认。

## 部署

### 前置条件

1. **Nginx 反代**：cache_dir 必须能通过公网 HTTP(S) 访问（Telegra.ph 需要拉图片）
2. **Telegram Bot Token**：从 @BotFather 申请
3. **Pixiv PHPSESSID**（可选，仅 R-18 需要）

### 安装

```bash
git clone <this-repo>
cd feed-bot
pip install -e .
cp config.example.yaml config.yaml
# 编辑 config.yaml，至少填：
#   telegram.token, auth.admin_users, publish.base_url, storage.*
python -m pixivfeed
```

首次启动会自动创建 Telegra.ph 账号并写回 `config.yaml`。

### Nginx 示例

```nginx
location /p/ {
    alias /var/cache/pixiv-feed-bot/images/;
    add_header Cache-Control "public, max-age=86400";
}
```

### 启用 e-hentai / ExHentai / nhentai

启动后私聊 bot：

```
/setting set collectors.ehentai.enabled true
/setting set collectors.nhentai.enabled true
/setting edit collectors.exhentai.ipb_pass_hash
/setting edit collectors.exhentai.ipb_member_id
/setting edit collectors.exhentai.igneous
/setting set collectors.exhentai.enabled true
```

完全无需重启。

### 本地 Bot API（处理大文件 / `/zip2tph` / `/archive` 必备）

Telegram 官方 Bot API 对 `getFile`（接收上传）有 20MB 上限，对 `sendDocument`（发送）有 50MB 上限。`/zip2tph` 接收用户上传的 zip、`/archive` 把图集打包回发，都很容易超过这两个上限。要绕开就必须自建 [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) 服务并在配置里启用 `local_mode`。

#### 1. 起一个 telegram-bot-api 服务（docker 示例）

```bash
docker run -d --name telegram-bot-api --restart unless-stopped \
  -p 127.0.0.1:8081:8081 \
  -e TELEGRAM_API_ID=<你的 api_id> \
  -e TELEGRAM_API_HASH=<你的 api_hash> \
  -e TELEGRAM_LOCAL=1 \
  -v /var/lib/telegram-bot-api:/var/lib/telegram-bot-api \
  -v /tmp/telegram-bot-api:/tmp/telegram-bot-api \
  aiogram/telegram-bot-api:latest \
  --local --dir=/var/lib/telegram-bot-api --temp-dir=/tmp/telegram-bot-api \
  --http-ip-address=0.0.0.0 --http-port=8081
```

`api_id` / `api_hash` 在 [my.telegram.org](https://my.telegram.org/) 申请（不是 Bot Token）。`--local` 参数缺一不可，否则即使自建也会按 20MB 限制处理。

#### 2. 把 bot 从官方 API 切到本地 API（关键步骤）

Telegram 服务端规定：bot 在切到本地 Bot API 前**必须**先在官方 API 调用一次 `logOut`，否则本地 API 会拒绝该 bot 的 `getFile` 等调用，表现为 `Not Found`。

```bash
# 1) 先停 feed bot
systemctl stop pixiv-feed-bot

# 2) 在官方 API 上 logOut（一次性，从此该 bot 必须用本地 API）
curl -sS "https://api.telegram.org/bot${BOT_TOKEN}/logOut"
# 期待：{"ok":true,"result":true}

# 3) 重启本地 telegram-bot-api 干净接管
docker restart telegram-bot-api
```

要换回官方 API，需要在本地 API 上再调一次 `logOut`。

#### 3. 在 `config.yaml` 启用

```yaml
telegram:
  token: "..."
  base_url: "http://127.0.0.1:8081/bot"
  base_file_url: "http://127.0.0.1:8081/file/bot"
  local_mode: true
```

`local_mode: true` 是 PTB 客户端开关，告诉 PTB 在 `getFile` 后直接读本地文件路径，绕开 HTTPS 拉一遍的 20MB 限制。三项必须同时正确，否则要么走回官方 API，要么报 `File is too big`。

#### 4. 文件读权限

`--local` 模式下 telegram-bot-api 把上传文件落盘到 `--dir`（默认 `/var/lib/telegram-bot-api/<token>/...`）。运行 feed bot 的 `pixivbot` 用户必须能读到这个目录。docker 默认 root 跑的话最简单的处理：

```bash
chmod -R o+rX /var/lib/telegram-bot-api
```

如果出现 `Permission denied` 就是这一步漏了。

#### 5. 启动并验证

```bash
systemctl start pixiv-feed-bot
journalctl -u pixiv-feed-bot -n 50 --no-pager
```

之后**重新发送一次 zip** 给 bot 触发 `/zip2tph` —— 之前的 `file_id` 是官方 API 发的，本地 API 不认。

## 模板变量

详见 `config.example.yaml` 里 `templates` 段的注释。

- `templates.illust.*` —— pixiv 插画
- `templates.novel.*` —— pixiv 小说
- `templates.gallery.*` —— eh / ex / nh 共用

## CLI（开发调试用）

```bash
python -m pixivfeed.provider.pixiv.cli illust 12345 --meta-only
python -m pixivfeed.provider.pixiv.cli publish-illust 12345
python -m pixivfeed.provider.pixiv.cli publish-novel 999
python -m pixivfeed.provider.pixiv.cli url "https://www.pixiv.net/artworks/12345"
```

## 许可证

MIT。

参考实现致谢：
- [DojinGo](https://github.com/...) 的 collector 抽象与 nhentai/eh 解析逻辑
- [telegram-bili-feed-helper](https://github.com/...) 的 Provider/Registry 架构形状
